/*! CoreApp v1.2.0 | Copyright by Softnio. */
...
function token_calc(a) {
    var b = $(a),
        ...
        E = 0,
        F = 0;
    b.is(".token-number") && (z = w, A = trim_number($.number(w * y, decimals.max, ".", "")), $(".pay-amount").val(parseFloat(A)), $(".pay-amount-u").text(parseFloat(A))), b.is(".pay-amount") && (A = w, z = trim_number($.number(w / y, decimals.min, ".", "")), $(".token-number").val(parseFloat(z)), $(".token-number-u").text(parseFloat(z))), 0 === z ? (token_alert(k, "info-circle", "icon"), l.addClass("disabled").removeAttr("data-amount")) : z >= minimum_token && z <= maximum_token ? (token_alert(k, "check-circle text-success", "icon"), token_alert(k, A * minimum_token, "amount"), token_alert(k, "", "text"), l.removeClass("disabled")) : z < minimum_token ? (token_alert(k, "times-circle text-danger", "icon"), l.addClass("disabled").removeAttr("data-amount")) : z > maximum_token && (token_alert(k, "Maximum you can purchase " + maximum_token + " token per contribution.", "text"), l.addClass("disabled").removeAttr("data-amount")), C = parseFloat(token_bonus(z, "base")), D = parseFloat(token_bonus(z, "amount")), E = parseFloat(token_bonus(z, "total")), F = parseFloat(z) + E, A = isNaN(A) ? 0 : A, F = isNaN(F) ? 0 : F;
    var G = trim_number($.number(F, decimals.min, ".", ""));
    token_alert(k, trim_number($.number(B, decimals.max, ".", "")), "amount"), e.text(x), f.text(E), g.text(C), h.text(D), d.text(trim_number($.number(A, decimals.max, ".", ""))), i.text(G), p.text(trim_number($.number(A, decimals.max, ".", ""))), q.text(x), r.text(E), s.text(G), t.val(x), v.val(z), ("btc" == x || "ltc" == x || "eth" == x) && u.text("\"Coingate\""), ("usd" == x || "eur" == x || "gbp" == x) && u.text("\"Paypal\"");
    var H = amount_bonus ? amount_bonus : {
        1: 0
    };
